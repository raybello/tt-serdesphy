...
async def mission_mode_traffic(dut):
    ...
    dut._log.info(
        f"FIFO Status: TX Full={status['tx_fifo_full']}, "
...
async def csr_register_readback(dut):
    ...
    for reg_addr, expected in test_patterns.items():
        ...
        if actual == expected:
            ...
            dut._log.error(
                f"  Reg 0x{reg_addr:02X}: FAIL (expected 0x{expected:02X}, got 0x{actual:02X})"
            )
            all_passed = False
    ...
    for reg_addr, value in new_patterns.items():
        ...
        if actual == value:
            ...
            dut._log.error(
                f"  Reg 0x{reg_addr:02X}: FAIL (expected 0x{value:02X}, got 0x{actual:02X})"
            )
            all_passed = False
